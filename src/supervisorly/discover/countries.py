"""Country-name → ISO 3166-1 alpha-2 resolution for the ``--country`` CLI flag.

The ROR filter needs an alpha-2 code, but the documented usage (README/SKILL) passes an
English country *name* (``--country Canada``). This module is the one translation step.

D-038 note: this table is **standards-body reference data** (ISO 3166-1 English short names
plus a few common exonyms/former names) — an enum of the world's countries, not a per-field
search-term dictionary or a university list. It classifies user input into a fixed standard
vocabulary; it generates no queries.
"""
...
def _fold(s: str) -> str:
    """Case/diacritic-insensitive fold so ``Côte d'Ivoire`` / ``Türkiye`` match ASCII input."""
    nfkd = unicodedata.normalize("NFKD", s)
    return "".join(c for c in nfkd if not unicodedata.combining(c)).casefold().strip()


# ISO 3166-1 alpha-2, keyed by the folded official English short name.
ISO_NAME_TO_CODE = {_fold(name): code for code, name in [
...
# Common variants, exonyms and former names people actually type (still fixed reference
# data, not generated vocabulary).
ISO_NAME_TO_CODE.update({_fold(name): code for code, name in [
    ("US", "United States"), ("US", "USA"), ("US", "America"),
    ("GB", "United Kingdom"), ("GB", "UK"), ("GB", "Great Britain"), ("GB", "Britain"),
    ("AE", "UAE"), ("KR", "South Korea"), ("KP", "North Korea"),
    ("RU", "Russia"), ("IR", "Iran"), ("SY", "Syria"), ("VE", "Venezuela"),
    ("BO", "Bolivia"), ("VN", "Vietnam"), ("LA", "Laos"), ("TZ", "Tanzania"),
    ("MD", "Moldova"), ("BN", "Brunei"), ("CI", "Ivory Coast"), ("CV", "Cape Verde"),
    ("TR", "Turkey"), ("TL", "East Timor"), ("VA", "Vatican"), ("VA", "Vatican City"),
    ("SZ", "Swaziland"), ("MM", "Burma"), ("NL", "Holland"), ("MO", "Macau"),
    ("TW", "Taiwan"), ("HK", "Hong Kong SAR"), ("PS", "Palestine"),
    ("CZ", "Czech Republic"), ("MK", "Macedonia"), ("CG", "Republic of the Congo"),
    ("CD", "DR Congo"), ("CD", "Democratic Republic of the Congo"),
]})
...
def to_country_code(value: str | None) -> str | None:
    """Resolve a ``--country`` value to ISO 3166-1 alpha-2, or None when unrecognized.

    A 2-letter code passes through as-is (uppercased); an English country name is looked up
    in the ISO table above. None means "fail loud" — the caller reports it and exits (D-002),
    never silently querying ROR with a filter that matches nothing.
    """
    v = (value or "").strip()
    if not v:
        return None
    if _CODE_RE.match(v):
        return v.upper()
    return ISO_NAME_TO_CODE.get(_fold(v))
