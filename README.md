...
## The hosted web app (optional)

Everything above is the CLI. There is also a **one-page web app** that wraps the same engine
for people who will not use a terminal: a 5-step wizard (you → field → topics → scope →
progress) that runs the scan as a background job you can watch, cancel and resume, and hands
back the same dashboard.

Run it locally — no cloud account needed. Two steps, because the dev server serves the API
only and the page is a generated artifact, not a checked-in file:

```bash
# 1. the API + a local threaded worker
python -m supervisorly.webapi --port 8765

# 2. the page, pointed at that API — then open output/webapp.html in a browser
...
(When deployed, that second step is unnecessary: Hosting rewrites serve the page from the
same origin as the API, so `api_base` stays empty.)

The page is generated by `supervisorly.export.webapp.build_webapp()` and is
self-contained: no CDN, no tracking, no external resource. The endpoints it uses are
`/api/expand`, `/api/map`, `/api/scan`, `/api/scan/<id>[/cancel|/resume]` and
`/api/result/<id>`; each is rate-limited per client and returns honest JSON errors.

Scan results are **personal data** ([D-005](docs/DECISIONS.md#d-005--ethics-in-code)): a job id
is an unguessable token, jobs are never listable, and nothing is stored in the browser.

To deploy it to Firebase (Hosting + Functions + Firestore + a Cloud Run worker), follow
[`firebase/README.md`](firebase/README.md) — an 8-step runbook. You supply your own project id,
region, results bucket and secrets; **nothing in this repo is pre-pointed at a live project.**
